...
import { AISWellBeingStudentsListColumns } from './studentWellBeingConstants'
import { handleDownloadExcelForStudentWellBeing } from './studentWellBeingThunk'
import { fetchStudentWellBeing } from './studentWellBeingFunctions'
import EditStudentWllBeingDialog from './EditStudentWllBeingDialog'
import { tableStyles } from '../../../components/styles/tableStyles'

const StudentWellBeingList = ({
	...
	const [columns, setColumns] = useState(AISWellBeingStudentsListColumns)
	...
	const renderCellContent = (column, row) => {
		...
		if (fieldName === 'wellBeingAssessmentSubmissionDate') {
			return row?.wellBeingAssessmentSubmissionDate
				? formatDate(row?.wellBeingAssessmentSubmissionDate)
				: row?.createdAt
			...
			{allStudentsForspecificSchool?.data?.length > 0 ? (
				<Box sx={{ ...tableStyles.container, flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, overflow: 'hidden' }}>
					...
					<Box sx={tableStyles.footer}>
						<Box
							...
							onClick={() => {
								const body = fetchStudentWellBeing(
									dispatch,
									filterData,
									searchText,
									currentPage,
								...
								handleDownloadExcelForStudentWellBeing(body, true)
							}}
						>
				...
				<Box sx={{ ...tableStyles.emptyState, flex: 1 }}>
					...
					<Typography sx={tableStyles.emptyStateTitle}>
						{localizationConstants.noStudentWellBeingMsg}
					</Typography>
				</Box>
			)}

			{modal.edit && (
				<EditStudentWllBeingDialog
					open={modal.edit}
...
export default StudentWellBeingList
