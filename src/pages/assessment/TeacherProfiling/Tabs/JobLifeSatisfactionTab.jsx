...
import {
	...
	questionsRatingForjobLifeSatisfaction,
} from '../teacherProfilingConstants'
import { teacherProfilingStyles } from '../teacherProfilingStyles'
import { setJobLifeSatisfactionScore } from '../teacherProfilingSlice'

const JobLifeSatisfactionTab = ({ isEditable }) => {
	const flexStyles = useCommonStyles()
	const dispatch = useDispatch()
	const { jobLifeSatisfactionScore } = useSelector(
		(store) => store.teacherProfiling,
	)

	const handleOnChangeRating = (index, option) => {
		const newScores = [...jobLifeSatisfactionScore]
		...
		dispatch(setJobLifeSatisfactionScore(newScores))
	}

	return (
		<Box>
			{jobLifeSatisfactionScore?.length > 0 ? (
				<>
					<Box
						sx={{
					...
					>
						{jobLifeSatisfactionAssessmentQuestions?.map(
							(questions, index) => (
								<Box
									sx={{
								...
								>
									...
									>
										...
										<Box>
											{questionsRatingForjobLifeSatisfaction.map(
												(option, index1) => (
													<FormControlLabel
														key={index1}
														checked={
...
export default JobLifeSatisfactionTab
