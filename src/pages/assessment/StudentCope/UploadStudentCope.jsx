...
import { StudentCopeUploadDefaultExcelColumns } from './StudentCopeConstants'
import { uploadStudentCopeData } from './StudentCopeFunction'
...
const UpdateStudentCope = ({ modal, handleModal, refreshList }) => {
			...
			<Box sx={{ display: 'flex', mb: '20px' }}>
				...
				>
					<Box sx={{ display: 'flex', gap: '3px' }}>
						<Typography variant={typographyConstants.body}>
							{' '}
							{localizationConstants.questions}{' '}
						</Typography>
						<Typography
							sx={{
								color: 'red',
				...
				<Box sx={{ width: '58%', ml: '10px' }}>
					<Typography variant={typographyConstants.body}>
						{' '}
						{localizationConstants.scoreInstructionMsg}{' '}
					</Typography>
				</Box>
			</Box>
		</>
		...
		<Box>
			<FileUploadDialog
				...
				handleExcelDownload={(e) => {
					e.stopPropagation()
					downloadExcel(
						StudentCopeUploadDefaultExcelColumns,
						'StudentCope.xlsx',
					)
				}}
				Content={content}
				...
				mandatoryKeys={bulkUploadMandatoryKeys.studentCOPERecords}
			/>
			<CustomDialog
				...
				onRightButtonClick={() => {
					ErrorMsgsExcelDownload(
						response,
						setDeleteBulkDialog,
						'Student COPE Validation Error.xlsx',
					)
				}}
			/>
		</Box>
...
export default memo(UpdateStudentCope)
